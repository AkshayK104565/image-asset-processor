"""
app.py  –  Pattern Image Asset Processor  (no OAuth)
"""
...
from core.engine import find_magick, parse_dim, run_job, validate_workbook, DEFAULT_WORKERS
...
def api_run():
    ...
    num_workers = int(data.get("num_workers", DEFAULT_WORKERS))
    ...
    with _jobs_lock:
        _jobs[job_id] = {"status":"running","queue":q,"result":None,"created":time.time()}
    def worker():
        def cb(cur, tot, fn):
            q.put({"type":"progress","current":cur,"total":tot,"file":fn})
        result = run_job(excel_path, upload_choice, min_size, max_size,
                         num_workers=num_workers, progress_cb=cb)
        with _jobs_lock:
...
@app.route("/api/download-failures/<job_id>")
def api_download_failures(job_id):
    """Download highlighted Excel showing failed URLs."""
    job = _job(job_id)
    if not job or job["status"] != "done":
        return jsonify({"error":"Job not ready."}), 404
    hp = job["result"].get("highlighted_excel_path","")
    if not hp or not os.path.isfile(hp):
        return jsonify({"error":"No failure report available."}), 404
    return send_file(hp,
                     mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                     as_attachment=True,
                     download_name="failed_links_highlighted.xlsx")
